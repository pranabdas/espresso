...
keywords: ["dielectric", "epsilon", "jdos", "dft"]
---
...
plt.show()
```
<img src={require("../../static/img/silicon-epsilon.webp").default} className="inv-hue-rot-180" alt="silicon-epsilon"/>
...
k-mesh. We usually need finer k-mesh for $\epsilon$ to converge. By default the
maximum number of k-points is set to 40000 in Quantum Espresso, if we need more
k-points, we can change [Modules/parameters.f90](
https://gitlab.com/QEF/q-e/-/blob/develop/Modules/parameters.f90) and recompile
Quantum Espresso.

:::

## Joint Density of States
JDOS gives the number of states for a pair of initial and final states as a
function of frequency (in eV). A larger intensity indicates a larger absorption
in the absorption spectra. Input file for JDOS calculation:

import jdos_si from '!!raw-loader!/src/silicon/epsilon.jdos.si.in';

<CodeBlock language="bash" title="src/silicon/epsilon.jdossi.in" showLineNumbers>{jdos_si}</CodeBlock>
...
<img src={require("../../static/img/silicon-epsilon-jdos.webp").default} className="inv-hue-rot-180" alt="silicon-epsilon-jdos"/>
