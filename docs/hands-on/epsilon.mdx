...
k-points option seems to work.

Here is a python script to generate k-points manually:

import gen_epsilon_k_grid from '!!raw-loader!/scripts/gen_epsilon_k_grid.py';

<CodeBlock language="bash" title="scripts/gen_epsilon_k_grid.py" showLineNumbers>{gen_epsilon_k_grid}</CodeBlock>

If required, we can perform `nscf` calculation with finer k-grid.
