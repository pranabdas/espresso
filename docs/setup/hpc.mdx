...
here are the steps that I followed to successfully compile. [Download](
https://github.com/QEF/q-e/releases) and decompress the source files.
```bash
...
<img
  src={require("/img/intel-link-line-adviser.png").default}
  alt="Intel link line adviser"
  width="600px"
/>
...
`qe-7.0/bin` path to your `.bashrc`:
```bash
echo 'export PATH="/home/svu/{username}/qe-7.0/bin:$PATH"' >> ~/.bashrc
```
...
executables, e.g., `/home/svu/{username}/qe-7.0/bin/pw.x`. PBS system won't read
your bash settings, neither the relative paths of your login node would apply.
