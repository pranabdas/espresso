...
<CodeBlock className="language-bash" title="src/pbs_job.sh">{pbs_job_sh}</CodeBlock>
...
`qe-6.8/bin` path to your `.bashrc`:
```bash
echo 'export PATH="/home/svu/{username}/qe-6.8/bin:$PATH"' >> ~/.bashrc
```
...
executables, e.g., `/home/svu/{username}/qe-6.8/bin/pw.x`. PBS system won't read
